...
export default function PaymentClient({ escrow }: { escrow: Escrow }) {
  ...
  const handleSubmitPayment = async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(t("payment.paymentSubmissionFailed"));
      const data = await res.json();
      setTxHash(data.txHash ?? data.transactionHash ?? data.hash ?? "mock_tx_hash");
      track("payment_completed", { escrowId: escrow.id });
    } catch (e: unknown) {
      setPaymentError(e instanceof Error ? e.message : t("payment.paymentFailed"));
    } finally {
  ...
  return (
    <div className="rounded-3xl border border-zinc-200 bg-white p-8 shadow-sm dark:border-zinc-800 dark:bg-zinc-950">
      ...
      {!isConnected ? (
        ...
        >
          {isLoading ? t("payment.connecting") : t("payment.connectWallet")}
        </button>
      ) : (
